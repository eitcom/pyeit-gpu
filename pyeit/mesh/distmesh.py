...
class DISTMESH:
    ...
    def __init__(
        ...
        self.deps = cp.sqrt(cp.finfo(cp.double).eps) * h0
        self.geps = 1e-1 * h0
        ...
        selection = cp.random.rand(p.shape[0]) < (r0 / cp.max(r0))
        p = p[selection]
        ...
        self.pold = cp.inf * cp.ones((self.N, self.n_dim))
    ...
    def is_retriangulate(self):
        ...
        return cp.max(dist(self.p - self.pold)) > (self.h0 * self.ttol)
    ...
    def triangulate(self):
        ...
        tri = Delaunay(self.p.get(), qhull_options="QJ").simplices
        pmid = cp.mean(self.p[tri], axis=1)

        # keeps only interior points
        t = tri[self.fd(pmid).get() < -self.geps]
        # extract edges (bars)
        ...
        self.bars = cp.asarray(np.unique(bars_tuple).view(bars.dtype).reshape((-1, 2)))
        self.t = t
    ...
    def bar_force(self, L, L0, barvec):
        ...
        # cols : x, y, x, y (2D)
        # : x, y, z, x, y, z (3D)
        data = cp.hstack([Fvec, -Fvec])
        if self.n_dim == 2:
            rows = self.bars[:, [0, 0, 1, 1]]
            cols = cp.dot(cp.ones(cp.shape(F)), cp.array([[0, 1, 0, 1]]))
        else:
    ...
    def density_control(self, L, L0, dscale=3.0):
        ...
        ixdel = cp.setdiff1d(self.bars[ixout, :].reshape(-1), cp.arange(self.nfix))
        self.p = self.p[cp.setdiff1d(cp.arange(self.N), ixdel)]
        # Nold = N
    ...
    def move_p(self, Ftot):
        ...
        self.num_move += 1
        # move p along forces
        self.p += cp.asarray(self.deltat) * Ftot
        ...
        delta_move = self.deltat * cp.max(dist(Ftot[d < -self.geps]))
...
def bbox2d_init(h0, bbox):
    ...
    """
    x, y = cp.meshgrid(
        cp.arange(bbox[0][0], bbox[1][0], h0),
        cp.arange(bbox[0][1], bbox[1][1], h0 * sqrt(3).get() / 2.0),
        indexing="xy",
    )
...
def bbox3d_init(h0, bbox):
    ...
    x, y, z = cp.meshgrid(
        cp.arange(bbox[0][0], bbox[1][0], xspace),
        cp.arange(bbox[0][1], bbox[1][1], yspace),
        cp.arange(bbox[0][2], bbox[1][2], zspace),
        indexing="xy",
    )
    ...
    y[:, :, 1::2] += h0 / (2.0 * sqrt(3))
    # p : Nx3 ndarray
    p = cp.array([x.ravel(), y.ravel(), z.ravel()]).T
    return p
