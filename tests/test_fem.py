...
def _assemble(ke, tri, perm, n):
    ...
    k = cp.zeros((n, n), dtype=perm.dtype)
...
def _mesh_obj():
    ...
    node = cp.array([[0.13, 0.15], [0.2, 0.2], [0.1, 0.1], [0.18, 0.12]])
    element = cp.array([[0, 2, 3], [0, 3, 1]])
    # assemble uses perm.dtype, perm MUST not be cp.int (result rounding error in K)
    perm = cp.array([3.0, 1.0])
    el_pos = cp.array([1, 2])
...
def _mesh_obj_large():
    ...
    node = cp.random.randn(n_pts, 2)
    element = cp.array([cp.random.permutation(n_pts)[:3] for _ in range(n_tri)])
    perm = cp.random.randn(n_tri)
    cp.random.seed(0)
    el_pos = cp.random.permutation(n_pts)[:16]
...
class TestFem(unittest.TestCase):
    def test_ke_triangle(self):
        ...
        pts = cp.array([[0, 1], [0, 0], [1, 0]])
        tri = cp.array([[0, 1, 2]])
        k_truth = cp.array([[1, -1, 0], [-1, 2, -1], [0, -1, 1]])
        area = 0.5
        ke = pyeit.eit.fem.calculate_ke(pts, tri)

        self.assertTrue(ke.shape == (1, 3, 3))
        self.assertTrue(cp.allclose(ke[0], k_truth * area))

    def test_ke_tetrahedron(self):
        """test ke calculation using tetrahedron (3D)"""
        pts = cp.array([[0, 0, 0], [0, 0, 1], [0, 1, 0], [1, 0, 0]])
        tri = cp.array([[0, 1, 2, 3]])
        k_truth = cp.array(
        ...
        self.assertTrue(cp.allclose(ke[0], k_truth * volumn))
    ...
    def test_assemble(self):
        ...
        n, ne = 10, 42
        pts = cp.arange(n)
        tri = cp.array([pts[cp.random.permutation(n)[:3]] for _ in range(ne)])
        perm = cp.random.randn(ne)
        se = cp.random.randn(ne, 3, 3)
        k_truth = _assemble(se, tri, perm, n)
        k = pyeit.eit.fem.assemble(se, tri, perm, n).toarray()

        self.assertTrue(cp.allclose(k, k_truth))
    ...
    def test_meas_pattern(self):
        ...
        for parser in ["meas_current", "fmmu", "rotate_meas"]:
            ex_lines = [cp.random.permutation(n_el)[:2] for _ in range(10)]
            for ex_line in ex_lines:
                ex_mat = cp.array([ex_line])
                # build protocol dict/dataset
                protocol = _protocol_obj(ex_mat, n_el, 1, parser)
                fwd = pyeit.eit.fem.EITForward(mesh, protocol)
                diff_truth = cp.asarray(_meas_pattern(ex_line, n_el, 1, parser))
                diff = fwd.protocol.meas_mat[0]

                assert cp.allclose(diff, diff_truth)

    def test_subtract_row(self):
        """calculate f[diff_op[0]] - f[diff_op[1]]"""
        n_exe = 10
        n_el = 16
        v = cp.random.randn(n_el)
        diff_pairs = cp.array([cp.random.permutation(n_el)[:2] for _ in range(n_exe)])
        vd_truth = cp.array([v[d[0]] - v[d[1]] for d in diff_pairs])
        vd = pyeit.eit.fem.subtract_row(v, diff_pairs)

        self.assertTrue(vd_truth.size == vd.size)
        self.assertTrue(cp.allclose(vd.ravel(), vd_truth.ravel()))
    ...
    def test_k(self):
        ...
        self.assertTrue(cp.allclose(k, k_truth, rtol=0.01))
    ...
    def test_compute_jac(self):
        ...
        # testing solve
        jac_truth = cp.array([[-0.25874523, -2.75529584], [0.25874523, 2.75529584]])
        jac, _ = fwd.compute_jac()
        self.assertTrue(cp.allclose(jac, jac_truth))
    ...
    def test_compute_b_matrix(self):
        ...
        b_truth = cp.array([[1, 1, 0, 1], [1, 1, 0, 1]])
        # fix ref to be exactly the one in mesh
        fwd = pyeit.eit.fem.EITForward(mesh, protocol)
        b = fwd.compute_b_matrix()
        self.assertTrue(cp.allclose(b, b_truth))
